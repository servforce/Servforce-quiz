...
def test_local_database_contract_is_consistent_across_docs_and_compose():
    ...
    expected = "postgresql+psycopg2://admin:pasword@127.0.0.1:5433/md_quiz"

    assert expected in readme
    assert env_values.get("DATABASE_URL") == expected
    assert "cp .env.example .env" in readme
    assert "POSTGRES_USER=admin" in env_example
    assert "POSTGRES_PASSWORD=pasword" in env_example
    assert "POSTGRES_DB=md_quiz" in env_example
    assert "POSTGRES_USER: ${POSTGRES_USER:-admin}" in compose
    assert "POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-pasword}" in compose
    assert "POSTGRES_DB: ${POSTGRES_DB:-md_quiz}" in compose
