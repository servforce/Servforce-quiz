...
> 约束：**只参考 `qml.md`**；除“候选人列表”外尽量不使用数据库——试卷解析内容、答题数据等都落到 **JSON 文件** 中；HTTP 使用 **Flask（Python）**；管理员可上传 Markdown 试卷并生成候选人唯一链接/二维码；候选人身份验证后在线作答；系统自动判分（客观题）+ 大模型判分（简答题）得到总分是否进入面试。
